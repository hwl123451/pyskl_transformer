...
class PoseDataset(BaseDataset):
    ...
    Modified to support .avi videos for NTU RGB+D dataset in RGBPoseConv3D.
    """
    ...
    def load_pkl_annotations(self):
        ...
        for item in data:
            ...
            # 1. 获取原始 ID/文件名 (pkl里可能是 frame_dir='S001...' 或 filename='S001...')
            ...
            # 5. 拼接完整的绝对路径
            # self.data_prefix 就是你 config 里的 data_root
            abs_path = osp.join(self.data_prefix, new_filename)

            # 6. 更新 item 字典
            item['filename'] = abs_path
            # 同时更新 frame_dir，防止 MMDecode 找不到路径时报错
            item['frame_dir'] = abs_path
