        ...
        for mod in results['modality']:
            ...
            if mod == 'RGB':
                ...
                if os.path.exists(results['filename']):
                    ...
                    del video_reader
                else:   # the directory contains a list of png
                    assert 'frame_dir' in results
                    frame_dir = pathlib.Path(results['frame_dir'])
                    frame_dir = frame_dir.parent / ('adl_rgb' if 'adl' in frame_dir.name else 'fall_rgb') / frame_dir.name
                    # the image path example:/adl-03-cam0-rgb/adl-03-cam0-rgb-012.png
                    img_paths = sorted(os.listdir(frame_dir))
                    # load .png from img_paths, use PIL and convert them into numpy.array
                    imgs = [
                            np.array(Image.open(pathlib.Path(frame_dir) / img_path))
                            for framd_id, img_path in enumerate(img_paths) if framd_id in frame_inds
                            ]
                results['imgs'] = imgs
            elif mod == 'Pose':
                assert 'keypoint' in results
                if results['keypoint'].shape[1] == 1:
                    results['keypoint'] = np.transpose(results['keypoint'], axes=[1, 0, 2, 3])
                    results['keypoint_score'] = np.transpose(results['keypoint_score'], axes=[1, 0, 2])
