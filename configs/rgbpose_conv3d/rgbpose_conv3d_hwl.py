...
backbone_cfg = dict(
    ...
    pose_pathway=dict(
        ...
        in_channels=17, # ntu60_hrnet.pkl 使用的是 17 点 COCO 格式
        base_channels=32,
        out_indices=(2, ),
        conv1_kernel=(1, 7, 7),
        conv1_stride=(1, 1),
...
# --- Pipeline 修改 ---
# 核心修改：在 MMDecode 之前添加 PrependFilename 来匹配你带 "_rgb.avi" 的物理文件
# 如果你已经按我之前的建议用 rename 把软连接改名了（去掉了 _rgb），则把下面的 suffix='_rgb.avi' 改为 suffix='.avi'
# --- Pipeline 修改 ---
# 确保删除了所有 PrependFilename

common_pipeline = [
    dict(type='MMUniformSampleFrames', clip_len=dict(RGB=8, Pose=32), num_clips=1),
    dict(type='MMDecode', decoder_type='decord'),
...
data = dict(
    ...
    train=dict(
        ...
        pipeline=train_pipeline), # 删除了 modality='RGB'
    val=dict(
        type=dataset_type,
        ann_file=ann_file,
        split='xsub_val',
        data_prefix=data_root,
        pipeline=val_pipeline), # 删除了 modality='RGB'
    test=dict(
        type=dataset_type,
        ann_file=ann_file,
        split='xsub_val',
        data_prefix=data_root,
        pipeline=test_pipeline)) # 删除了 modality='RGB'
